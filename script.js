...
// Every case answers the questions a future client actually has: what needed
// solving, what made it difficult, what the bureau took responsibility for,
// and what changed as a result.
const projectProofs = {
  'project-casa-mare': {
    task: 'Создать дом для большой семьи на склоне у моря: открытый горизонту, но защищённый от посторонних взглядов и жаркого ветра.',
    constraints: 'Сложный рельеф, несколько поколений под одной крышей, жёсткие видовые оси соседних участков и активное солнце.',
    decisions: 'Разделили дом на три террасы; спрятали приватные комнаты в каменном основании; сделали внутренний двор и пассивную защиту от перегрева.',
    outcome: 'У семьи появился единый дом с разными степенями приватности: общая жизнь раскрыта к морю, личная — защищена и тиха.',
    scope: ['Архитектурная концепция', 'Интерьер и свет', 'Ландшафт и двор', 'Индивидуальные изделия', 'Авторское сопровождение']
  },
  'project-skyline': {
    task: 'Превратить пентхаус с панорамным остеклением в спокойный дом для коллекционера, не потеряв главный актив — вид на город.',
    constraints: 'Непрерывный фасад, насыщенная инженерия, вечерние блики и необходимость разместить искусство без музейной холодности.',
    decisions: 'Собрали сервисные функции в тёмное ядро; выстроили низкую мебель по горизонту; настроили независимые сценарии света и ниши для коллекции.',
    outcome: 'Панорама остаётся главным произведением, а интерьер работает и для ежедневной жизни, и для камерных приёмов.',
    scope: ['Перепланировка', 'Интерьер и комплектация', 'Световые сценарии', 'Bespoke-мебель', 'Размещение искусства']
  },
  'project-forest-house': {
    task: 'Разместить загородную резиденцию в лесу так, чтобы дом давал комфорт круглый год и не превращал участок в строительную площадку.',
    constraints: 'Существующие сосны и каменные гряды, северный климат, перепад высот и потребность семьи в одном уровне без длинных переходов.',
    decisions: 'Разбили программу на низкие павильоны; обошли деревья пятном застройки; связали части общей кровлей и защищёнными переходами.',
    outcome: 'Дом живёт в ритме леса: каждая зона получает свой вид, а маршрут семьи остаётся коротким, тёплым и понятным в любой сезон.',
    scope: ['Генплан и архитектура', 'Интерьер', 'Ландшафт', 'Климатическая стратегия', 'Надзор на площадке']
  },
  'project-atelier-17': {
    task: 'Адаптировать историческую парижскую квартиру для жизни коллекционера, сохранив характер анфилады и добавив современный комфорт.',
    constraints: 'Охраняемые пропорции, следы времени, необходимость скрыть технику и хранение, требования к свету для искусства.',
    decisions: 'Восстановили логику анфилады; объединили хранение, технику и свет в столярную оболочку; настроили оптику под каждую работу.',
    outcome: 'Историческая квартира не стала декорацией: она получила современную инфраструктуру и остаётся живым, личным пространством.',
    scope: ['Стратегия реставрации', 'Интерьер', 'Столярные изделия', 'Свет', 'Комплектация и искусство']
  },
  'project-fjord-retreat': {
    task: 'Создать небольшой отель на побережье, в котором суровый климат превращается в часть впечатления гостя, а не в проблему.',
    constraints: 'Ветер и осадки, открытый ландшафт, сезонность, необходимость сохранить человеческий масштаб и экономику небольшого объекта.',
    decisions: 'Собрали отель из небольших корпусов; организовали дворы-укрытия; выстроили маршрут от тёмных переходов к общим пространствам у моря.',
    outcome: 'Гость чувствует не изоляцию, а защищённость: погода становится сценарием отдыха, а архитектура поддерживает сервис и атмосферу.',
    scope: ['Архитектура', 'Интерьеры номеров', 'Гостевой маршрут', 'Ландшафт', 'FF&E и комплектация']
  },
  'project-courtyard-house': {
    task: 'Сделать резиденцию в пустыне комфортной для большой семьи без зависимости от визуально тяжёлых климатических решений.',
    constraints: 'Экстремальное солнце, перепады температур, приватность семьи, дефицит тени и воды в открытом ландшафте.',
    decisions: 'Построили дом вокруг последовательности дворов; использовали массу земляных стен, воду и ночное проветривание; разделили уровни доступа.',
    outcome: 'Роскошь ощущается через прохладу, тишину и защищённые внешние комнаты — дом работает с климатом, а не борется с ним.',
    scope: ['Архитектура', 'Климатическая стратегия', 'Интерьер', 'Ландшафт и вода', 'Авторский надзор']
  }
};

Object.entries(projectProofs).forEach(([projectId, proof]) => {
  const viewer = document.getElementById(projectId);
  const credits = viewer?.querySelector('.project-viewer__credits');
  if (!viewer || !credits) return;
  const scope = proof.scope.map((item) => `<li>${item}</li>`).join('');
  const section = document.createElement('section');
  section.className = 'project-proof';
  section.setAttribute('aria-label', 'Ценность работы бюро');
  section.innerHTML = `
    <div class="project-proof__lead">
      <span>03 / Ценность проекта</span>
      <h3>От запроса клиента — к <em>контролируемому результату.</em></h3>
    </div>
    <div class="project-proof__rows">
      <article><span>Задача клиента</span><p>${proof.task}</p></article>
      <article><span>Ограничения</span><p>${proof.constraints}</p></article>
      <article><span>Решения бюро</span><p>${proof.decisions}</p></article>
    </div>
    <div class="project-proof__outcome"><span>Результат для клиента</span><p>${proof.outcome}</p></div>
    <div class="project-proof__scope"><span>За что отвечает бюро</span><ul>${scope}</ul></div>
  `;
  credits.before(section);
});
